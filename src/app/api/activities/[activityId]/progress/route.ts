...
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    const {
      isCompleted,
      score,
      pointsEarned: clientPointsEarned,
      answers,
    } = body;
    ...
    const pointsEarned =
      clientPointsEarned ||
      (score ? Math.round((score / 100) * activity.points) : 0);
    ...
    try {
      ...
      if (isCompleted) {
        ...
        if (userToUpdate) {
          ...
          if (pointsToAdd > 0) {
            console.log(
              `Updating user ${user.id} points: ${
                userToUpdate.totalPoints
              } + ${pointsToAdd} = ${userToUpdate.totalPoints + pointsToAdd}`
            );
      ...
      if (isCompleted) {
        ...
        const totalPointsEarned = completedActivities.reduce(
          (sum: number, progress: { pointsEarned: number }) =>
            sum + progress.pointsEarned,
          0
        );
