...
export default function MFADemoLab({ activity, userId, progress }: MFADemoLabProps) {
  ...
  const handleSubmitLab = async () => {
    try {
      ...
      const pointsEarned = Math.round((score / (mfaTasks.reduce((sum, task) => sum + task.points, 0))) * activity.points);
              ...
              {showResults && (
                <div className="bg-green-50 border border-green-100 rounded-md p-4">
                  ...
                  <p className="text-green-700 mt-1">
                    You've earned {score} out of {mfaTasks.reduce((sum, task) => sum + task.points, 0)} points.
                  </p>
                </div>
              )}
